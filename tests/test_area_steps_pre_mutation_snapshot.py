...
def test_area_steps_use_pre_mutation_snapshot(tmp_path: Path) -> None:
    ...
    logger._area_snapshots_by_step_area[(1, int(area.unique_id))] = {
        ...
        "dist_to_ref_utilitarian": 0.11,
        "dist_to_ref_egalitarian": 0.22,
        "dist_to_ref_rawlsian": 0.33,
    }
    ...
    assert np.isclose(float(row["dist_to_ref_utilitarian"]), 0.11)
    assert np.isclose(float(row["dist_to_ref_egalitarian"]), 0.22)
    assert np.isclose(float(row["dist_to_ref_rawlsian"]), 0.33)
