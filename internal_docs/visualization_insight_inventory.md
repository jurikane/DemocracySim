...
This document defines an insight-first inventory for simulation visualizations.
It maps thesis intent and model concepts to concrete plot families.
...
The following list is the primary structure for plot decisions.
Plots are implementation choices; insights define the required evidence.
...
| `I-04` | Is puzzle-vs-power tension present and does anti-monopoly behavior hold? | quality diagnostics and model concept of puzzle/power conflict | `debug_doe` (primary), `thesis_run` (optional appendix) | decomposition + anti-monopoly diagnostics page | Strong | Keep in debug; optional in thesis |
...
| `I-09` | Do stability and anti-monopoly diagnostics indicate substantive behavior, not artifacts? | robustness interpretation for DOE diagnostics | `debug_doe` | explicit anti-monopoly traces + support counts + lock-in/chaos context | Medium | Keep, attach support context |
...
| `P-A-GATE-01` | `Puzzle Anti-Monopoly Diagnostics` | `I-04`, `I-09` | Keep | Optional appendix | Keep |
...
- retain puzzle/power/anti-monopoly pages
...
1. Keep this insight inventory as the working reference.
2. Implement `P-A-LEARN-CAUSAL-01` data fields and page.
3. Merge/remove redundant learning proxy pages (`P-A-GRP-05`, `P-A-GRP-06`) after equivalence checks.
